...
public class ControllerSalesPage {
	...
	void addItem(ActionEvent event) throws IOException {
		...
		if (itemIdAdd != -1) {
			...
			SalesPageConnection salesDB = new SalesPageConnection();
			boolean isInShop = salesDB.checkIsInShop(itemIdAdd, currentUser.getShopId());
			...
			if (isInShop) {
				...
				if (success) {
					...
					FXMLLoader loader = new FXMLLoader(getClass().getResource("/application/AlertDialog.fxml"));
					ControllerAlertDialog control = new ControllerAlertDialog(120, 210, "Error",
							"El art�culo se ha a�adido correctamente.");
					loader.setController(control);
					Parent root = loader.load();
					...
					FXMLLoader loader = new FXMLLoader(getClass().getResource("/application/AlertDialog.fxml"));
					ControllerAlertDialog control = new ControllerAlertDialog(120, 210, "Error",
							"Se ha producido un error. Por favor, int�ntelo m�s tarde.");
					loader.setController(control);
					Parent root = loader.load();
				...
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/application/AlertDialog.fxml"));
				ControllerAlertDialog control = new ControllerAlertDialog(120, 210, "Error",
						"No hay stock del art�culo en tienda. Revise los datos.");
				loader.setController(control);
				Parent root = loader.load();
	...
	void cancel(ActionEvent event) throws IOException {
		...
		ControllerYesNoAlertDialog control = new ControllerYesNoAlertDialog(0, 0, "Atenci�n",
				...
				"�Est� seguro de que desea cancelar esta venta?", "S�", "No", "ticketDelete", params);
		loader.setController(control);
		Parent root = loader.load();
